<?php
	function database()
	{
		if(!@$sql = mysqli_connect('localhost','root','','western_movie'))
		{
			echo 'NO CONNECT...!!';
			die('');
		}
	...
	{
		$sql = database();
		$film_name = mysql_real_escape_string($film_name);
		$film_content = mysql_real_escape_string($film_content);
		$film_category = mysql_real_escape_string($film_category);
		$film_genere = mysql_real_escape_string($film_genere);
		$film_refrence_title = mysql_real_escape_string($film_refrence_title);
		$film_refrence_url = mysql_real_escape_string($film_refrence_url);
		$image_file = mysql_real_escape_string($image_file);
		$film_file = mysql_real_escape_string($film_file);
		$film_year = mysql_real_escape_string($film_year);
	...
	function edit_film($id, $film_name, $film_content, $film_category, $film_genere, $film_refrence_title, $film_refrence_url, $make_year, $image_file, $film_file)
	{
		...
		$film_refrence_title = mysql_real_escape_string($film_refrence_title);
		$film_refrence_url = mysql_real_escape_string($film_refrence_url);
		$make_year = mysql_real_escape_string($make_year);
		$image_file = mysql_real_escape_string($image_file);
		$film_file = mysql_real_escape_string($film_file);
	...
	{
		$sql = database();
		$genere_name = mysql_real_escape_string($genere_name);
	...
	{
		$sql = database();
		$film_id = mysql_real_escape_string($film_id);
		$subtitle_content = mysql_real_escape_string($subtitle_content);
		$subtitle_file = mysql_real_escape_string($subtitle_file);
	...
	{
		$sql = database();
		$old_password = sha1(mysql_real_escape_string($old_password));
		$password = sha1(mysql_real_escape_string($password));
